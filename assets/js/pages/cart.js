document.addEventListener('DOMContentLoaded', () => {
  ...
  const render = () => {
    const rawCart = SportStore.read(SportStore.storage.cart);
    ...
    const rows = validItems
      .map((item) => {
        ...
        if (!product || product.price == null || product.image == null || product.name == null) return '';

        const linePrice = Number(product.price) * item.qty;
        if (Number.isFinite(linePrice)) sum += linePrice;
